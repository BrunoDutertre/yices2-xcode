...
void nra_plugin_add_lp_variable_from_term(nra_plugin_t* nra, term_t t) {
  ...
  assert(int_hmap_find(&nra->lp_data.lp_to_mcsat_var_map, (int32_t) lp_var) == NULL);
  assert(int_hmap_find(&nra->lp_data.mcsat_to_lp_var_map, mcsat_var) == NULL);

  int_hmap_add(&nra->lp_data.lp_to_mcsat_var_map, (int32_t) lp_var, mcsat_var);
  int_hmap_add(&nra->lp_data.mcsat_to_lp_var_map, mcsat_var, (int32_t) lp_var);
}
...
variable_t nra_plugin_get_variable_from_lp_variable(nra_plugin_t* nra, lp_variable_t lp_var) {
  int_hmap_pair_t* find = int_hmap_find(&nra->lp_data.lp_to_mcsat_var_map, (int32_t) lp_var);
  assert(find != NULL);
  return find->val;
}
