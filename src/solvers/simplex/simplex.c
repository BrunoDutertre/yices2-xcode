 ...
 * To enable the debug code, set DEBUGGING to 1
...
#if TRACE || DEBUGGING || DUMP || TRACE_INIT || TRACE_PROPAGATION || TRACE_BB || TRACE_INTFEAS || !defined(NDEBUGGING)
...
/*
 * DEBUGGINGging functions are defined at the end of this file
 */
#if DUMP
...
#ifndef NDEBUGGING
/*
 * For DEBUGGINGging: check whether delta is between s->lb and s->ub
 */
...
#ifndef NDEBUGGING
/*
 * For DEBUGGINGging: check that all the integer variables that don't have an integer value
 * are basic.
 */
...
#ifndef NDEBUGGING
/*
 * For DEBUGGINGging: go through the column of variable x
...
static bool simplex_make_integer_feasible(simplex_solver_t *solver) {
   ...
   */
  nbounds = solver->bstack.top;
  if (/* DISABLES CODE */ (false) && !simplex_intfeas_strengthening(solver)) return false;
  if (!simplex_intfeas_integrality_constraints(solver)) return false;
  if (/* DISABLES CODE */ (false) && !simplex_intfeas_diophantine_check(solver)) return false;
  if (/* DISABLES CODE */ (false) && !simplex_intfeas_strengthening(solver)) return false;
   ...
   */
  if (/* DISABLES CODE */ (false) && solver->bstack.top > nbounds && !simplex_intfeas_strengthening(solver)) {
    return false;
  }
  ...
  if (solver->stats.num_branch_atoms >= 20) {
    if (/* DISABLES CODE */ (false) && v->size > 1 && bb_score > 200000000 && solver->stats.num_gomory_cuts < 100) {
...
bool simplex_propagate(simplex_solver_t *solver) {
   ...
   */
  if (/* DISABLES CODE */ (false) && solver->check_counter -- <= 0) {
...
/*
 * DEBUGGINGGING OF MODEL CONSTRUCTION
 */
