...
/*
 * Set these flags to 1 for DEBUGGINGging, trace, data collection
 */
#define DEBUGGING 0
#define TRACE 0
#define DATA  0
...
static void nsat_reduce_learned_clause_set(sat_solver_t *solver) {
  ...
  check_candidate_clauses_to_delete(solver, a, n); // DEBUGGING
...
static void prepare_for_search(sat_solver_t *solver) {
  ...
  check_clause_pool_counters(&solver->pool);      // DEBUGGING
  check_watch_vectors(solver);                    // DEBUGGING
}
...
/*
 * For DEBUGGINGging: show the definition of variable x
 */
static void show_var_def(const sat_solver_t *solver, bvar_t x) {
  ttbl_t tt;
   uint32_t i;
...
/****************************************
 *   CONSISTENCY CHECKS FOR DEBUGGINGGING   *
 ***************************************/
