...
static inline int32_t free_list_next(int32_t i) {
  return (int32_t) mpz_get_si(mpq_numref(bank_q[i]));
}
...
int q_set_from_float_string(rational_t *r, const char *s) {
  ...
  resize_string_buffer((uint32_t) len + 1);
  c = *s ++;
...
void q_gcd(rational_t *r1, const rational_t *r2) {
  ...
  if (r2->den != 0) {
    if (r1->den != 0) {
      ...
      b = abs32(r2->num);
      d = (uint32_t) mpz_gcd_ui(NULL, mpq_numref(bank_q[r1->num]), b);
      free_mpq(r1->num);
    }
    ...
    if (r1->den != 0) {
      ...
      a = abs32(r1->num);
      d = (uint32_t) mpz_gcd_ui(NULL, mpq_numref(bank_q[r2->num]), a);
      assert(d <= MAX_NUMERATOR);
      r1->num = d;
      r1->den = 1;
    } else {
...
void q_integer_rem(rational_t *r1, rational_t *r2) {
  ...
  if (r2->den != 0) {
    ...
    if (r1->den != 0) {
      ...
      // r1 is gmp, r2 is a small integer
      n = (int32_t) mpz_fdiv_ui(mpq_numref(bank_q[r1->num]), r2->num);
